...
void eval(char *cmdline) 
{
	...
	if(sigprocmask(SIG_BLOCK, &mask, 0) != 0) unix_error("SigProcMask Error");
	...
	// BG
	if(isBg) printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline);
...
{	
	...
	if(strcmp(argv[0], "bg") == 0) {
		...
		job->state = BG;
		if(kill(-(job->pid), SIGCONT) != 0) unix_error("SigCont Error");
	}
...
void sigint_handler(int sig) 
{
	...
	if(pid == 0) return;
	if(kill(-pid, SIGINT) != 0) unix_error("SigInt Error");
...
void sigtstp_handler(int sig) 
{
	pid_t pid = fgpid(jobs);
	if(pid == 0) return;
	if(kill(-pid, SIGTSTP) != 0) unix_error("SigTstp Error");
